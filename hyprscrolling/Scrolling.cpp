...
std::any CScrollingLayout::layoutMessage(SLayoutMessageHeader header, std::string message) {
    ...
    if (ARGS[0] == "move") {
        ...
        if (ARGS[1][0] == '+' || ARGS[1][0] == '-') {
            ...
            if (m_config.configuredWidths[m_config.currentIdx % m_config.configuredWidths.size()] == WDATA->column->columnWidth) {
                ++m_config.currentIdx;
            }

            WDATA->column->columnWidth = m_config.configuredWidths[m_config.currentIdx++ % m_config.configuredWidths.size()];
        } else {
            float abs = 0;
            try {
                abs = std::stof(ARGS[1]);
