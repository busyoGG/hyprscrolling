...
void SColumnData::add(PHLWINDOW w, int after) {
  ...
  windowDatas.insert(
      windowDatas.begin() + after + 1,
      makeShared<SScrollingWindowData>(w, self.lock(),
                                       1.F / (float)(windowDatas.size() + 1)));
}
...
void CScrollingLayout::applyNodeDataToWindow(SP<SScrollingWindowData> data,
                                             bool force, bool hasWindowsRight,
                                             bool hasWindowsLeft) {
  PHLMONITOR PMONITOR;
...
void CScrollingLayout::onEnable() {
  ...
  m_configCallback = g_pHookSystem->hookDynamic(
      "configReloaded", [this](void *hk, SCallbackInfo &info, std::any param) {
        // bitch ass
  ...
  m_focusCallback = g_pHookSystem->hookDynamic(
      "activeWindow", [this](void *hk, SCallbackInfo &info, std::any param) {
        const auto PWINDOW = std::any_cast<PHLWINDOW>(param);

        if (!PWINDOW)
          return;

        static const auto PFOLLOW_FOCUS =
            CConfigValue<Hyprlang::INT>("plugin:hyprscrolling:follow_focus");
...
void CScrollingLayout::onWindowCreatedTiling(PHLWINDOW window,
  ...
  if (m_columnMoveState.isMovingColumn &&
      window->m_workspace->m_id == m_columnMoveState.targetWorkspaceID)
    return;
  ...
  if (!droppingColumn) {
    ...
    if (window->m_draggingTiled) {
      ...
      if (droppingOn) {
        ...
        // 判断鼠标是否在中间 60% 区域内
        bool isInMiddle60Percent =
            (mouseX >= leftLimit && mouseX <= rightLimit);

        if (isInMiddle60Percent) {
          // 如果鼠标在中间 60% 区域内，按照原来方式放置窗口
          const auto TOP =
              droppingOn->getWindowIdealBoundingBoxIgnoreReserved().middle().y >
              mouseY;
...
void CScrollingLayout::resizeActiveWindow(const Vector2D &delta,
  ...
  if (!DATA) {
    *PWINDOW->m_realSize =
        (PWINDOW->m_realSize->goal() + delta)
            .clamp(PWINDOW->m_ruleApplicator->minSize().valueOr(
                       Vector2D{MIN_WINDOW_SIZE, MIN_WINDOW_SIZE}),
                   PWINDOW->m_ruleApplicator->maxSize().valueOr(
  ...
  if (DATA->column->windowDatas.size() > 1) {
    ...
    case CORNER_TOPRIGHT: {
      ...
      if (PREV_WD->windowSize <= MIN_ROW_HEIGHT && modDelta.y <= 0 ||
          CURR_WD->windowSize <= MIN_ROW_HEIGHT && delta.y >= 0)
        break;

      float adjust = std::clamp((float)(modDelta.y / USABLE.h),
...
SP<SScrollingWindowData>
CScrollingLayout::findBestNeighbor(SP<SScrollingWindowData> pCurrent,
                                   SP<SColumnData> pTargetCol) {
  ...
  for (const auto &candidate : pTargetCol->windowDatas) {
    ...
    const double candidateBottom =
        candidate->layoutBox.y + candidate->layoutBox.h;
    const bool overlaps =
  ...
  if (!overlappingWindows.empty()) {
    ...
    auto topmost = std::ranges::min_element(
        overlappingWindows, std::less<>{},
        [](const SP<SScrollingWindowData> &w) { return w->layoutBox.y; });
    return *topmost;
  }
...
std::any CScrollingLayout::layoutMessage(SLayoutMessageHeader header,
                                         std::string message) {
  static auto centerOrFit = [](const SP<SWorkspaceData> WS,
                               const SP<SColumnData> COL) -> void {
    static const auto PFITMETHOD =
        CConfigValue<Hyprlang::INT>("plugin:hyprscrolling:focus_fit_method");
    if (*PFITMETHOD == 1)
      WS->fitCol(COL);
  ...
  if (ARGS[0] == "move") {
    ...
    if (ARGS[1] == "+col" || ARGS[1] == "col") {
      ...
      g_pCompositor->warpCursorTo(
          COL->windowDatas.front()->window.lock()->middle());
      ...
      if (!WDATA) {
        if (DATA->leftOffset <= DATA->maxWidth() && DATA->columns.size() > 0) {
          ...
          DATA->recalculate();
          focusWindowUpdate(
              (DATA->columns.back()->windowDatas.back())->window.lock());
          g_pCompositor->warpCursorTo((DATA->columns.back()->windowDatas.back())
                                          ->window.lock()
                                          ->middle());
        }
    ...
    focusWindowUpdate(ATCENTER ? (*ATCENTER->windowDatas.begin())->window.lock()
                               : nullptr);
    ...
    CScopeGuard x([WDATA] {
      WDATA->column->columnWidth = std::clamp(
          WDATA->column->columnWidth, MIN_COLUMN_WIDTH, MAX_COLUMN_WIDTH);
    ...
    if (ARGS[1] == "active") {
      ...
      for (size_t i = 0; i < WDATA->columns.size(); ++i) {
        ...
        WDATA->columns[i]->columnWidth =
            1.F / (float)(WDATA->columns.size() - i);
      }
    ...
    auto col = idx == -1 ? WDATA->column->workspace->add()
                         : WDATA->column->workspace->add(idx);
    ...
    if (direction == "l")
      ...
      target_idx = (current_idx == (int64_t)col_count - 1) ? current_idx
                                                           : (current_idx + 1);
    else
      return {};
    ...
    if (arg.starts_with("+") || arg.starts_with("-")) {
      try {
        ...
        if (!PWORKSPACE)
          PWORKSPACE = g_pCompositor->createNewWorkspace(targetWorkspaceID,
                                                         PMONITOR->m_id);
      } catch (...) {
        return {};
      }
      ...
      if (!PWORKSPACE)
        PWORKSPACE = g_pCompositor->createNewWorkspace(
            SPECIAL_WORKSPACE_ID, PMONITOR->m_id, "special");
    } else {
      ...
      if (!PWORKSPACE) {
        try {
          ...
          PWORKSPACE =
              g_pCompositor->createNewWorkspace(0, PMONITOR->m_id, arg);
    ...
    if (!targetWorkspaceData) {
      targetWorkspaceData = m_workspaceDatas.emplace_back(
          makeShared<SWorkspaceData>(PWORKSPACE, this));
    ...
    for (const auto &ws : m_workspaceDatas) {
      ...
      const auto focusedColumn = (focusedData && focusedData->column &&
                                  focusedData->column->workspace.lock() == ws)
                                     ? focusedData->column.lock()
                                     : nullptr;
      ...
      if (toggled == 1) {
        ...
        for (const auto &col : ws->columns) {
          const double itemWidth = *PFSONONE && ws->columns.size() == 1
                                       ? USABLE.w
                                       : USABLE.w * col->columnWidth;
        ...
        const auto columnToCenter =
            focusedColumn ? focusedColumn : fallbackColumn;
        if (!columnToCenter)
          continue;
  ...
  if (dir == "l") {
    ...
    if (!COL) {
      ...
      if (COL->windowDatas.size() > 0)
        COL->add(DATA, COL->idxForHeight(
                           g_pInputManager->getMouseCoordsInternal().y));
      else
        COL->add(DATA);
