...
struct SScrollingWindowData {
  SScrollingWindowData(PHLWINDOW w, SP<SColumnData> col, float ws = 1.F)
      : window(w), column(col), windowSize(ws) {
    ;
  }
...
struct SWorkspaceData {
  SWorkspaceData(PHLWORKSPACE w, CScrollingLayout *l)
      : workspace(w), layout(l) {
    ;
  }
...
public:
  ...
  virtual void resizeActiveWindow(const Vector2D &,
                                  eRectCorner corner = CORNER_NONE,
                                  PHLWINDOW pWindow = nullptr);
  virtual void
  fullscreenRequestForWindow(PHLWINDOW pWindow,
...
private:
  ...
  SP<SScrollingWindowData> findBestNeighbor(SP<SScrollingWindowData> pCurrent,
  ...
  void applyNodeDataToWindow(SP<SScrollingWindowData> node, bool instant,
                             bool hasWindowsRight, bool hasWindowsLeft);
